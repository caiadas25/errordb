...
func greet() string {
  ...
  // === Go ===
  {
    ...
    language: "Go",
    category: "RuntimeError",
    explanation: "You're calling a method or accessing a field on a nil pointer. This happens when a pointer variable hasn't been initialized, or when an interface holds a nil value and you call methods on it.",
    causes: [
      "Using a pointer that was never assigned a value (var p *T)",
      "Calling a method on a nil interface",
      "Forgetting to check if a function returned nil before using the result",
      "Accessing a map value without checking if the key exists (returns zero value)"
    ],
    solutions: [
      "Initialize pointers with `&Type{}` or `new(Type)`",
      "Always check for nil before using pointers: `if p != nil { ... }`",
      "For interfaces, check both the type and value: `if iface != nil { ... }`",
      "Use the comma-ok pattern for maps: `val, ok := m[key]`"
    ],
    codeExample: `// ❌ Bad — p is nil
var p *int
fmt.Println(*p) // panic!
...
p = new(int)
*p = 42`,
    relatedErrors: ["go-assignment-to-nil-map", "go-index-out-of-range"]
  },
  {
    id: "go-assignment-to-nil-map",
    errorMessage: "assignment to entry in nil map",
    language: "Go",
    category: "RuntimeError",
    explanation: "You're trying to write to a map that hasn't been initialized. Unlike reading from a nil map (which returns the zero value), writing to a nil map panics immediately.",
    causes: [
      "Declaring a map with `var m map[string]int` without initializing",
      "Passing a nil map to a function that writes to it",
      "Forgetting to call `make(map[...])` before using the map"
    ],
    solutions: [
      "Initialize maps with `make(map[K]V)` or map literal `map[K]V{}`",
      "Always check if the map is nil before writing",
      "Use the comma-ok idiom to check if a key exists before updating"
    ],
    codeExample: `// ❌ Bad — m is nil
...
m := map[string]int{"key": 1}`,
  ...
  {
    id: "go-index-out-of-range",
    errorMessage: "runtime error: index out of range [N] with length L",
    language: "Go",
    category: "RuntimeError",
    explanation: "You're accessing an array or slice at an index that doesn't exist. The index is either negative or greater than or equal to the length of the collection.",
    causes: [
      "Off-by-one error in loop bounds (using <= instead of <)",
      "Accessing a slice index that was expanded but not checked",
      "Using a random index without bounds checking",
      "Slicing past the end of a slice"
    ],
    solutions: [
      "Always check the length before accessing by index: `if i < len(s) { ... }`",
      "Use range-based loops: `for i, v := range s`",
      "Validate slice lengths from external sources (APIs, files)",
      "Use `s[:len(s)]` or `s[:]` carefully — verify bounds"
    ],
    codeExample: `// ❌ Bad — off-by-one
for i := 0; i <= len(s); i++ {
  fmt.Println(s[i]) // panics on last iteration
}
